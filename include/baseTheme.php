...
if(isset($_SERVER['PATH_INFO'])) {
	$_SERVER['REQUEST_URI'] = substr($_SERVER['REQUEST_URI'], strpos($_SERVER['PATH_INFO']));
}

//$temp = $_SERVER[SCRIPT_NAME];
$_SERVER['SCRIPT_NAME'] = htmlspecialchars($_SERVER['SCRIPT_NAME']);

$_SESSION["token"] = md5(uniqid(mt_rand(), true));

if ($is_adminOfCourse and isset($currentCourseID)) {
	//$eclass_module_id = oSanitize($eclass_module_id);
