...
elseif ($a == 1)  {
	if (isset($add)) {
		...
		if (empty($codefaculte) or empty($faculte)) {
			...
			<a href=\"$_SERVER[SCRIPT_NAME]?a=1\">".$langReturnToAddFaculte."</a></p></center>";
			}
		// Check for greek letters
		elseif (!preg_match("/^[A-Z0-9a-z_-]+$/", $codefaculte)) {
			$tool_content .= "<p>".$langGreekCode."</p><br />";
			$tool_content .= "<center><p><a href=\"$_SERVER[SCRIPT_NAME]?a=1\">".$langReturnToAddFaculte."</a></p></center>";
			}
		// Check if faculty code already exists
		elseif (mysql_num_rows(mysql_query("SELECT * from faculte WHERE code=" . autoquote(dbSanitize($codefaculte)))) > 0) {
			$tool_content .= "<p>".$langFCodeExists."</p><br />";
			$tool_content .= "<center><p><a href=\"$_SERVER[SCRIPT_NAME]?a=1\">".$langReturnToAddFaculte."</a></p></center>";
			}
		// Check if faculty name already exists
		elseif (mysql_num_rows(mysql_query("SELECT * from faculte WHERE name=" . autoquote(dbSanitize($faculte)))) > 0) {
			$tool_content .= "<p>".$langFaculteExists."</p><br />";
			$tool_content .= "<center><p><a href=\"$_SERVER[SCRIPT_NAME]?a=1\">".$langReturnToAddFaculte."</a></p></center>";
		} else {
		// OK Create the new faculty
			mysql_query("INSERT into faculte(code,name,generator,number) VALUES(" . autoquote(idbSanitize($codefaculte)) . ',' . autoquote((idbSanitize($faculte))) . ",'100','1000')")
				or die ($langNoSuccess);
			$tool_content .= "<p>".$langAddSuccess."</p><br />";
			}
	} else {
		// Display form for new faculte information
		$tool_content .= "<form method=\"post\" action=\"".$_SERVER['SCRIPT_NAME']."?a=1\">";
	...
	$tool_content .= "<br /><p align='right'><a href='$_SERVER[SCRIPT_NAME]'>".$langBack."</a></p>";
}
// Delete faculty
elseif ($a == 2) {
        $c = intval($_GET['c']);
...
elseif ($a == 3)  {
	...
	if (isset($_POST['edit'])) {
		...
		if (empty($faculte)) {
			$tool_content .= "<p>".$langEmptyFaculte."</p><br>";
			$tool_content .= "<p align='right'><a href='$_SERVER[SCRIPT_NAME]?a=3&c=$c'>$langReturnToEditFaculte</a></p>";
			}
		// Check if faculte name already exists
		elseif (mysql_num_rows(mysql_query("SELECT * from faculte WHERE id <> ".dbSanitize($c)." AND name=" .
                                                   autoquote(dbSanitize($faculte)))) > 0) {
			$tool_content .= "<p>".$langFaculteExists."</p><br>";
			$tool_content .= "<p align='right'><a href='$_SERVER[SCRIPT_NAME]?a=3&amp;c=$c'>$langReturnToEditFaculte</a></p>";
		} else {
		// OK Update the faculte
			mysql_query("UPDATE faculte SET name = " .
                                   autoquote(idbSanitize($faculte)) . " WHERE id=$c")
				or die ($langNoSuccess);
		// For backwards compatibility update cours and cours_facult also
			db_query("UPDATE cours SET faculte = " .
                                    autoquote(idbSanitize($faculte)) . " WHERE faculteid=$c")
				or die ($langNoSuccess);
			db_query("UPDATE cours_faculte SET faculte = " .
                                    autoquote(idbSanitize($faculte)) . " WHERE facid=$c")
				or die ($langNoSuccess);
		...
		$tool_content .= "<form method='post' action='$_SERVER[SCRIPT_NAME]?a=3'>";
		$tool_content .= "<table width='99%' class='FormData'>
		<tbody>
		<tr>
		<th width='220'>&nbsp;</th>
