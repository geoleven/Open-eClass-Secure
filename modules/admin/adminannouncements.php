...
if (isset($_GET['delete'])) {
	...
	// submit announcement command
        if (isset($_POST['id'])) {
                // modify announcement
                $id = intval($_POST['id']);
                $title = iSanitize($title);
                $newContent = iSanitize($newContent);
                $comment = iSanitize($comment);
                $title_en = iSanitize($title_en);
                $newContent_en = iSanitize($newContent_en);
                $comment_en = iSanitize($comment_en);
                db_query("UPDATE admin_announcements
                        SET gr_title = $title, gr_body = ". autoquote(iSanitize($newContent)) .", gr_comment = $comment,
                        en_title = $title_en, en_body = ". autoquote(iSanitize($newContent_en)) .", en_comment = $comment_en,
