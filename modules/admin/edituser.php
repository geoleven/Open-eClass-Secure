	...
	draw($tool_content, 3, ' ', $head_content);
 	 ...
 	 elseif(isset($user_exist) AND $user_exist == TRUE) {
		...
		if($registered_at>$expires_at) {
			...
			$sql = "UPDATE user SET nom = ".autoquote(idbSanitize($lname)).", prenom = ".autoquote(idbSanitize($fname)).",
				username = ".autoquote(idbSanitize($username)).", email = ".autoquote(idbSanitize($email)).", 
				statut = ".idbSanitize(intval($newstatut)).", phone=".autoquote(idbSanitize($phone)).",
				department = ".idbSanitize(intval($department)).", expires_at=".idbSanitize($expires_at).",
                                am = ".autoquote(idbSanitize($am))." WHERE user_id = ".idbSanitize(intval($u));
			$qry = db_query($sql);
                        if (!$qry) {
		...
		$tool_content .= "<h1>Wrong captcha!</h1>\n<p><a href=\"listcours.php\">$back</p>\n";
	}
}
}
else
