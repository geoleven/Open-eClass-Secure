...
if (isset($submit))  {
  ...
  $titulaires = idbSanitize($titulaires);
  $intitule = idbSanitize($intitule);
  // Update query
	...
	if (mysql_affected_rows() > 0) {
		$sql = mysql_query("UPDATE cours_faculte SET faculte='$facname', facid='$facid' WHERE code='".dbSanitize($_GET['c'])."'");
...
// Display edit form for course basic information
	...
	$row = mysql_fetch_array(mysql_query("SELECT * FROM cours WHERE code='".dbSanitize($_GET['c'])."'"));
