...
if (isset($search) && $search=="yes") {
	...
	if(!empty($searchtitle)) {
		$searchcours[] = "intitule LIKE '%".dbSanitize($searchtitle)."%'";
	}
	if(!empty($searchcode)) {
		$searchcours[] = "code LIKE '%".dbSanitize($searchcode)."%'";
	}
	if ($searchtype != "-1") {
		$searchcours[] = "visible = '".dbSanitize($searchtype)."'";
	}
	if($searchfaculte != "0") {
		$searchcours[] = "faculte = '".dbSanitize($searchfaculte)."'";
	}
	$searchcours = dbSanitize($searchcours);
