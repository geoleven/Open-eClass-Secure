 ...
 // Display Actions Toolbar
  $tool_content .= "
	  ...
	  <li><a href='newuseradmin.php$linkget'>$linkreg</a></li>
          ...
          <li><a href='$_SERVER[SCRIPT_NAME]?show=accepted$reqtype'>$langReqHaveFinished</a></li>
        </ul>
      </div>";
...
if (!empty($show) && ($show=="closed")) {
	if (!empty($id) && ($id>0)) {
		...
		$tool_content .= "<tbody>";
		$list_statut = dbSanitize($list_statut);
		...
		while ($req = mysql_fetch_array($sql)) {
			...
			$tool_content .= "<td align=center>
			<a href='$_SERVER[SCRIPT_NAME]?id=$req[rid]&show=closed$reqtype'>$langRestore</a></td>\n  </tr>";
		$k++;
		}
	}
	$tool_content .= "\n  </tbody>\n  </table>\n";
		...
		while ($req = mysql_fetch_array($sql)) {
			...
			$tool_content .= "<td align=\"center\">
				<small>".nice_format(date("Y-m-d", strtotime($req['date_open'])))."</small></td>";
                	$tool_content .= "<td align=\"center\">
				<small>".nice_format(date("Y-m-d", strtotime($req['date_closed'])))."</small></td>";
                	$tool_content .= "<td>".iSanitize($req['comment'])."</td>";
			$tool_content .= "<td align=center>
			<a href='$_SERVER[SCRIPT_NAME]?id=$req[rid]&show=closed$reqtype'>$langRestore</a>
			</td></tr>";
			$k++;
		}
	}
...
$langEmail: $emailhelpdesk";
					...
					$tool_content .= "<br><p><b>$langComments:</b><br />".iSanitize($comment)."</p>\n";
				}
			}
		} else {
			// display the form
			...
			<textarea class='auth_input' name='comment' rows='5' cols='60'>".iSanitize($d['comment'])."</textarea>
			</td></tr>
...
else
{
	...
	while ($req = mysql_fetch_array($sql)) {
		...
		$tool_content .= "<td align='center'>
			<small>".nice_format(date("Y-m-d", strtotime($req['date_open'])))."</small></td>";
		$tool_content .= "<td align='center'>".iSanitize($req[comment])."</td>";
		$tool_content .= "<td align='center'>
		<a href='$_SERVER[SCRIPT_NAME]?id=$req[rid]&amp;close=1$reqtype' onclick='return confirmation();'>$langClose</a><br />
		<a href='$_SERVER[SCRIPT_NAME]?id=$req[rid]&amp;close=2$reqtype'>$langRejectRequest</a>";
...
if (!empty($show)) {
	$tool_content .= "<p align=\"right\"><a href=\"$_SERVER[SCRIPT_NAME]\">$langBackRequests</a></p><br>";
}
