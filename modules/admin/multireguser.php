...
if (isset($_POST['submit']) && $securimage->check($_POST['captcha_code']) != false) {
        ...
        $send_mail = iSanitize($send_mail);
        $unparsed_lines = '';
        $new_users_info = array();
        $newstatut = ($_POST['type'] == 'prof')? 1: 5;
        $newstatut = ($newstatut);
        $facid = intval($_POST['facid']);
        $facid = iSanitize($facid);
        $am = $_POST['am'];
        $am = iSanitize($am);
        $fields = iSanitize($fields);
        $fields = preg_split('/[ \t,]+/', $_POST['fields'], -1, PREG_SPLIT_NO_EMPTY); 
        foreach ($fields as $field) {
                $field = iSanitize($field);
                if (!in_array($field, $acceptable_fields)) {
                        $tool_content = "<p class='caution_small'>$langMultiRegFieldError <b>".iSanitize($field)."</b></p>";
        ...
        while ($line !== false) {
                ...
                if (!empty($line)) {
                        ...
                        if (count($user) >= $numfields) {
                                $info = array();
                                foreach ($fields as $field) {
                                        $info[$field] = iSanitize(array_shift($user));
                                }
...
function create_user($statut, $uname, $nom, $prenom, $email, $depid, $am, $phone, $lang, $send_mail)
{
		...
		$nom = iSanitize($nom);
		$prenom = iSanitize($prenom);
		$email = iSanitize($email);
		$depid = iSanitize($depid);
		$am = iSanitize($am);
		$phone = iSanitize($phone);
		$statut = iSanitize($statut);
        ...
        $req = db_query('SELECT * FROM user WHERE username = ' . autoquote(dbSanitize($uname)));
        ...
        $req = db_query("INSERT INTO user
                        ...
                        VALUES (" .
				autoquote(iSanitize($nom)) . ', ' .
				autoquote(iSanitize($prenom)) . ', ' .
				autoquote(iSanitize($uname)) . ", '$password_encrypted', " .
				autoquote(iSanitize($email)) .
				", $statut, $depid, " .
                                "$registered_at, $expires_at, '$lang', " .
                                autoquote(iSanitize($am)) . ', ' .
                                autoquote(iSanitize($phone)) . ')');
        $id = mysql_insert_id();
...
";
        ...
        return array($id, $nom, $prenom, $email, iSanitize($phone), $am, $uname, $password);
}
...
function create_username($statut, $depid, $nom, $prenom, $prefix)
{
        ...
        do {
                ...
                $lastid++;
        } while (user_exists(oSanitize($uname)));
        return iSanitize($uname);
}


function register($uid, $course_code)
{
        $code = autoquote(dbSanitize($course_code));
