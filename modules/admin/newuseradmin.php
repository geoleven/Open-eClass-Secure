...
if($submit && $securimage->check($_POST['captcha_code']) != false) {
	// register user
	...
	$username_check = mysql_query("SELECT username FROM `$mysqlMainDb`.user 
			WHERE username=".autoquote(dbSanitize($uname)));
	...
	if (!$all_set) {
		...
		$inscr_user = db_query("INSERT INTO `$mysqlMainDb`.user
				...
				VALUES (" .
				autoquote(iSanitize($nom_form)) . ', ' .
				autoquote(iSanitize($prenom_form)) . ', ' .
				autoquote(iSanitize($uname)) . ", '$password_encrypted', " .
				autoquote(iSanitize($email_form)) .
				", $pstatut, $depid, " . autoquote(iSanitize($comment)) . ", $registered_at, $expires_at, '$proflanguage')");
...
";
	...
	<td><input class='FormData_InputText' type='password' name='password''></td>
	</tr>
	<tr>
