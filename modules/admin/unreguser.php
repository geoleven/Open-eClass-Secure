        ...
        if (!$c) {
                if ($u == 1) {
                        ...
                        if ($total>0) {
                                ...
                                $q = db_query("DELETE from user WHERE user_id = " . intval(dbSanitize($u)));
                        ...
                        if($u!=1) {
                                db_query("DELETE from admin WHERE idUser = '".dbSanitize($u)."'");
                        }
                ...
                $q = db_query("DELETE from cours_user WHERE user_id = $u AND
                                        cours_id = (SELECT cours_id FROM cours WHERE code = ".quote(dbSanitize($c)).")");
...
function get_uid_statut($u)
{
	...
	if ($r = mysql_fetch_row(db_query("SELECT statut FROM user WHERE user_id = '".dbSanitize($u)."'",	$mysqlMainDb)))
	{
		return $r[0];
	}
	else
