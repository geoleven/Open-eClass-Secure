...
{
	$qry = "SELECT * FROM auth WHERE auth_id = '".dbSanitize($auth)."'";
	$result = db_query($qry);
	if($result) {
...
function auth_user_login ($auth, $test_username, $test_password)  {
	...
	case '1':
	    ...
	    $sql = "SELECT user_id FROM user WHERE username='".dbSanitize($test_username)."' 
		AND password='".dbSanitize($test_password)."'";
	    $result = db_query($sql);
	    if(mysql_num_rows($result)==1) {
    		$testauth = true;
	    } else {
