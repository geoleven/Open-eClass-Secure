...
//	$tool_content .= "<table width=\"99%\"><tbody><tr><td>";
	...
	$sql=db_query("SELECT * FROM cours_user WHERE cours_id = (SELECT cours_id FROM cours WHERE code = "  . quote(dbSanitize($_POST['cc'])) . ")");

	while ($m = mysql_fetch_array($sql)) {

		$sql1 = db_query("SELECT email FROM user WHERE user_id='".dbSanitize($m["user_id"])."'");
