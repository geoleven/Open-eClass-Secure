...
$userName = iSanitize($userName);
// $userName = iSanitize($userName);
//$email = iSanitize($email);

$securimage = new Securimage();	
if (isset($_REQUEST['do']) && $_REQUEST['do'] == "go" && $securimage->check($_POST['captcha_code']) != false) {
	$userUID = (int)$_REQUEST['u'];
	$hash = $_REQUEST['h'];
	$res = db_query("SELECT `user_id`, `hash`, `password`, `datetime` FROM passwd_reset
			WHERE `user_id` = '" . dbSanitize($userUID) . "'
			AND `hash` = '" . dbSanitize($hash) . "'
	...
	if (mysql_num_rows($res) == 1) {
		...
		$sql = "UPDATE `user` SET `password` = '".$myrow['hash']."' WHERE `user_id` = ".dbSanitize($myrow['user_id'])."";
		if(db_query($sql, $mysqlMainDb)) {
			//send email to the user of his new pass (not hashed)
			$res = db_query("SELECT `email` FROM user WHERE `user_id` = ".dbSanitize($myrow['user_id'])."", $mysqlMainDb);
...
} elseif (!isset($_REQUEST['do'])&&$securimage->check($_POST['captcha_code']) != false) {
	...
	$res = db_query("SELECT user_id, nom, prenom, username, password, statut FROM user
			WHERE email = '" . dbSanitize($email) . "'
			AND BINARY username = '" . dbSanitize($userName) . "'", $mysqlMainDb);
	...
	/***** Account details found, now send e-mail *****/
        if ($found_editable_password) {
       	...
       	$tempmail = oSanitize($email);
       	$userName = iSanitize($userName);
       	$userName = oSanitize($userName);
		$tool_content .= "<table width=\"99%\"><tbody>
		<tr><td class=\"caution\">
		<p><strong>$langAccountNotFound1 ($userName / $tempmail)</strong></p>
