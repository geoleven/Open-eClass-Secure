...
if (!isset($submit)) {
	...
	// check if the username is already in use
		$q2 = "SELECT username FROM `$mysqlMainDb`.user WHERE username='".dbSanitize($uname)."'";
	...
	if (count($registration_errors) == 0 && $securimage->check($_POST['captcha_code']) != false) {
	...
	$tool_content .= "<table width='99%'><tbody><tr>" .
		...
		$_POST[email] = iSanitize($_POST[email]);
		$_POST[am] = iSanitize($_POST[am]);
		$tool_content .= "<p><a href='$_SERVER[SCRIPT_NAME]?prenom_form=$_POST[prenom_form]&nom_form=$_POST[nom_form]&uname=$_POST[uname]&email=$_POST[email]&am=$_POST[am]'>$langAgain</a></p>" .
