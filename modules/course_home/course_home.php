...
$uid = dbSanitize($uid);
$REMOTE_ADDR = dbSanitize($REMOTE_ADDR);
$currentCourse = dbSanitize($currentCourse);
...
while($result = mysql_fetch_row($res)) {
    ...
    $description = oSanitize($description);
	$keywords = trim($result[1]);
    $keywords = oSanitize($keywords);
	$addon = nl2br(trim($result[2]));
    $addon = oSanitize($addon);
	$faculte = $result[3];
    $faculte = oSanitize($faculte);
	$type = $result[5];
    $type = oSanitize($type);
	$visible = $result[6];
    $visible = oSanitize($visible);
	$professor = $result[7];
    $professor = oSanitize($professor);
	$fake_code = $result[8];
    $fake_code = oSanitize($fake_code);
}
...
if ($is_adminOfCourse) {
        if (isset($_REQUEST['edit_submit'])) {
                ...
                if (isset($_REQUEST['unit_id'])) { // update course unit
                        ...
                        $cours_id = dbSanitize($cours_id);
                        $unit_id = dbSanitize($unit_id);
                        $langCourseUnitModified = dbSanitize($langCourseUnitModified);
		        ...
		        $main_content .= "\n      <p class='success_small'>$langCourseUnitModified</p>";
                        ...
                        $descr = dbSanitize($descr);
                        $order = dbSanitize($order);
                        $cours_id = dbSanitize($cours_id);
                        $langCourseUnitAdded = dbSanitize($langCourseUnitAdded);
...
while ($cu = mysql_fetch_array($sql)) {
                ...
                $cu[id] = oSanitize($cu[id]);
                $cu[title] = oSanitize($cu[title]);
                ...
                if ($is_adminOfCourse) { // display actions
                        $cunits_content .= "\n        <td width='2%' style=\"border-bottom: 1px solid #CAC3B5;\">".
                                ...
                                "\n        <td width='2%' style=\"border-bottom: 1px solid #CAC3B5;\"><a href='$_SERVER[SCRIPT_NAME]?del=$cu[id]' " .
                                "onClick=\"return confirmation();\">" .
                                "<img src='../../template/classic/img/delete.gif' " .
                                "title='$langDelete'></img></a></td>" .
                                "\n        <td width='2%' style=\"border-bottom: 1px solid #CAC3B5;\"><a href='$_SERVER[SCRIPT_NAME]?vis=$cu[id]'>" .
                                "<img src='../../template/classic/img/$icon_vis' " .
                                "title='$langVisibility'></img></a></td>";
                        if ($cu['id'] != $last_id) {
                                $cunits_content .= "\n        <td width='2%' style=\"border-bottom: 1px solid #CAC3B5;\"><a href='$_SERVER[SCRIPT_NAME]?down=$cu[id]'>" .
                                "<img src='../../template/classic/img/down.gif' title='$langDown'></img></a></td>";
                        } else {
                                $cunits_content .= "\n        <td width='2%' style=\"border-bottom: 1px solid #CAC3B5;\">&nbsp;&nbsp;&nbsp;&nbsp;</td>";
                        }
                        if (!$first) {
                                $cunits_content .= "\n        <td width='2%' style=\"border-bottom: 1px solid #CAC3B5;\"><a href='$_SERVER[SCRIPT_NAME]?up=$cu[id]'><img src='../../template/classic/img/up.gif' title='$langUp'></img></a></td>";
                        } else {
                                $cunits_content .= "\n        <td width='2%' style=\"border-bottom: 1px solid #CAC3B5;\">&nbsp;&nbsp;&nbsp;&nbsp;</td>";
                        }
                }
