...
include_once "../../include/cSanitize.php";
...
if (isset($_POST['submit']) && $securimage->check($_POST['captcha_code']) != false) {
        if (empty($_POST['title'])) {
                ...
                $newlang = dbSanitize($newlang);
                $cours_id = dbSanitize($cours_id);
                db_query("UPDATE `$mysqlMainDb`.cours
                          SET intitule = " . autoquote(idbSanitize($_POST['title'])) .",
                              faculte = " . autoquote(idbSanitize($facname)) . ",
                              description = " . autoquote(idbSanitize($_POST['description'])) . ",
                              course_addon = " . autoquote(idbSanitize($_POST['course_addon'])) . ",
                              course_keywords = ". autoquote(idbSanitize($_POST['course_keywords'])) . ",
                              visible = " . intval(dbSanitize($_POST['formvisible'])) . ",
                              titulaires = " . autoquote(idbSanitize($_POST['titulary'])) . ",
                              languageCourse = '$newlang',
                              type = " . autoquote(idbSanitize($_POST['type'])) . ",
                              password = " . autoquote(idbSanitize($_POST['password'])) . ",
                              faculteid = " . intval(idbSanitize($facid)) . "
                          WHERE cours_id = $cours_id");
                $facname = dbSanitize($facname);
                $facid = dbSanitize($facid);
                ...
                $currentCourseID = dbSanitize($currentCourseID);
                $db = dbSanitize($db);
                $langAgenda = dbSanitize($langAgenda);
                $langLinks = dbSanitize($langLinks);
                $langDoc = dbSanitize($langDoc);
                $langVideo = dbSanitize($langVideo);
                $langWorks = dbSanitize($langWorks);
                $langAnnouncements = dbSanitize($langAnnouncements);
                $langAdminUsers = dbSanitize($langAdminUsers);
                $langForums = dbSanitize($langForums);
                $langExercices = dbSanitize($langExercices);
                $langModifyInfo = dbSanitize($langModifyInfo);
                $langGroups = dbSanitize($langGroups);
                $langDropBox = dbSanitize($langDropBox);
                $langConference = dbSanitize($langConference);
                $langCourseDescription = dbSanitize($langCourseDescription);
                $langQuestionnaire = dbSanitize($langQuestionnaire);
                $langLearnPath = dbSanitize($langLearnPath);
                $langUsage = dbSanitize($langUsage);
                $langToolManagement = dbSanitize($langToolManagement);
                $langWiki = dbSanitize($langWiki);
                $langCourseUnits = dbSanitize($langCourseUnits);
