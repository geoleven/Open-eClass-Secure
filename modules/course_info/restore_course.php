...
// -------------------------------------
	...
	<br /><br />
	<form action='".$_SERVER['SCRIPT_NAME']."' method='post' name='sendZip' enctype='multipart/form-data'>
	<input type='file' name='archiveZipped' />
	<br><br />
	<img id=\"captcha1\" src=\"../../include/securimage/securimage_show.php\" alt=\"CAPTCHA Image1\" />
    <input type=\"text\" name=\"captcha_code\" size=\"10\" maxlength=\"6\" />
	<a href=\"#\" onclick=\"document.getElementById('captcha1').src = '../../include/securimage/securimage_show.php?' + Math.random(); return false\">[ Different Image ]</a>
	<br><br />
	...
	<br><br />
	<img id=\"captcha2\" src=\"../../include/securimage/securimage_show.php\" alt=\"CAPTCHA Image2\" />
    <input type=\"text\" name=\"captcha_code\" size=\"10\" maxlength=\"6\" />
	<a href=\"#\" onclick=\"document.getElementById('captcha2').src = '../../include/securimage/securimage_show.php?' + Math.random(); return false\">[ Different Image ]</a>
	<br><br />
...
function announcement($text, $date, $order, $title = '') {
	...
	if (!$action) return;
	$title = dbSanitize($title);
	$text = dbSanitize($text);
	$date = dbSanitize($date);
	$new_course_id = dbSanitize($new_course_id);
	$order = dbSanitize($order);
...
function course_units($title, $comments, $visibility, $order, $resource_units) {
	...
	$mysqlMainDb = dbSanitize($mysqlMainDb);
	$title = dbSanitize($title);
	$comments = dbSanitize($comments);
	$visibility = dbSanitize($visibility);
	$order = dbSanitize($order);
	$new_course_id = dbSanitize($new_course_id);
...
function user($userid, $name, $surname, $login, $password, $email, $statut, $phone, $department, $registered_at = NULL, $expires_at = NULL, $inst_id = NULL) {
	...
	if (mysql_num_rows($u) > 0) 	{
		...
		$mysqlMainDb = dbSanitize($mysqlMainDb);
		$name = dbSanitize($name);
		$surname = dbSanitize($surname);
		$login = dbSanitize($login);
		$password = dbSanitize($password);
		$email = dbSanitize($email);
		$statut = dbSanitize($statut);
		$phone = dbSanitize($phone);
		$department = dbSanitize($department);
		$registered_at = dbSanitize($registered_at);
		$expires_at = dbSanitize($expires_at);
	...
	$mysqlMainDb = dbSanitize($mysqlMainDb);
	$new_course_id = dbSanitize($new_course_id);
	$userid_map[$userid] = dbSanitize($userid_map[$userid]); //????
	$statut = dbSanitize($statut);
...
function group($userid, $team, $status, $role) {
	...
	mysql_select_db($new_course_code);
	$userid_map[$userid] = dbSanitize($userid_map[$userid]);
	$team = dbSanitize($team);
	$status = dbSanitize($status);
	$role = dbSanitize($role);
...
function dropbox_file($userid, $filename, $filesize, $title, $description, $author, $uploadDate, $lastUploadDate) {
	...
	mysql_select_db($new_course_code);
	$userid_map[$userid] = dbSanitize($userid_map[$userid]);
	$filename = dbSanitize($filename);
	$filesize = dbSanitize($filesize);
	$title = dbSanitize($title);
	$description = dbSanitize($description);
	$author = dbSanitize($author);
	$uploadDate = dbSanitize($uploadDate);
	$lastUploadDate = dbSanitize($lastUploadDate);
...
function dropbox_person($fileId, $personId) {
	...
	$fileId = dbSanitize($fileId);
	$userid_map[$personId] = dbSanitize($userid_map[$personId]);
...
function dropbox_post($fileId, $recipientId) {
	...
	mysql_select_db($new_course_code);
	$fileId = dbSanitize($fileId);
	$userid_map[$recipientId] = dbSanitize($userid_map[$recipientId]);
...
function create_course($code, $lang, $title, $desc, $fac, $vis, $prof, $type) {
	...
	$repertoire = dbSanitize($repertoire);
	$lang = dbSanitize($lang);
	$title = dbSanitize($title);
	$desc = dbSanitize($desc);
	$fac = dbSanitize($fac);
	$vis = dbSanitize($vis);
	$prof = dbSanitize($prof);
	$code = dbSanitize($code);
...
function unpack_zip_show_files($zipfile)
{
	...
	while ($entries = readdir($handle)) {
		...
		if (is_dir($dirnameCourse.$entries))
			...
			<form action='".$_SERVER['SCRIPT_NAME']."' method='post' name='restoreThis'>
			<ol>";
