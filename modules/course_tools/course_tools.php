...
if ($is_adminOfCourse){
	...
	if  (isset($_REQUEST['toolStatus']) ){
		...
		$prevAgendaStateSQL = dbSanitize($prevAgendaStateSQL);
		$dbname = dbSanitize($dbname);
	...
	if ($securimage->check($_POST['captcha_code']) == true) {
			...
			if(isset($submit) &&  @$action == 2) {
				...
				//$link = quote(idbSanitize($link));
				//$name_link = quote(idbSanitize($name_link));

				$link = quote(dbSanitize(strip_tags($link)));
				$name_link = quote(idbSanitize(strip_tags($name_link)));
				
				$mID = dbSanitize($mID);
				// $name_link = idbSanitize($name_link);
				//$link = idbSanitize($link);
		...
		// -------------------------
			...
			if(isset($submit) &&  @$action == 1){
				...
				if (isset($file_name) and ($file_name != "") && ($file_size <= "$size") and ($link_name != "")) {
					...
					$link_name = quote(idbSanitize($link_name));
					$file_name = idbSanitize($file_name);
		...
		//$tool_content .= "<strong>Wrong captcha! Please retry.<br><br/></strong>";
	}
}
