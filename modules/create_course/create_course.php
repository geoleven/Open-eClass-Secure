...
if (isset($_POST['create_course'])) {
	...
	// to be deleted, static course database
	 $repertoire = "TMD100";
		...
		$new_cours_id = dbSanitize($new_cours_id);
		$uid = dbSanitize($uid);
		$faculte = dbSanitize($faculte);
		$repertoire = dbSanitize($repertoire);
		$facid = dbSanitize($facid);
        db_query("INSERT INTO cours SET
                        code = '$code',
                        languageCourse =" . quote(idbSanitize($language)) . ",
                        intitule = " . quote(idbSanitize($intitule)) . ",
                        description = " . quote(idbSanitize($description)) . ",
                        course_addon = " . quote(idbSanitize($course_addon)) . ",
                        course_keywords = " . quote(idbSanitize($course_keywords)) . ",
                        faculte = " . quote(idbSanitize($facname)) . ",
                        visible = " . quote(idbSanitize($formvisible)) . ",
                        titulaires = " . quote(idbSanitize($titulaires)) . ",
                        fake_code = " . quote(idbSanitize($code)) . ",
                        type = " . quote(idbSanitize($type)) . ",
                        faculteid = '$facid',
                        first_create = NOW()");
