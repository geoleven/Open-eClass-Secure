	...
	function _createNewWork ($uploaderId, $title, $description, $author, $filename, $filesize) {
		...
		*/
		...
		if ($GLOBALS['language'] == 'greek') {
			$this->filename = dbSanitize($this->filename);
		...
		*/
		if ($this->isOldWork) {
			...
			$this->filesize = dbSanitize($this->filesize);
			$this->title = dbSanitize($this->title);
			$this->description = dbSanitize($this->description);
			$this->author = dbSanitize($this->author);
			$this->lastUploadDate = dbSanitize($this->lastUploadDate);
			$this->id = dbSanitize($this->id);
			...
			$this->author = dbSanitize($this->author);
			$this->lastUploadDate = dbSanitize($this->lastUploadDate);
			$this->uploaderId = dbSanitize($this->uploaderId);
			$this->filename = dbSanitize($this->filename);
...
class Dropbox_Person {
	...
	function Dropbox_Person ($userId, $isCourseAdmin, $isCourseTutor) {
		...
		*/
		$this->userId = dbSanitize($this->userId);
		$currentCourseID = dbSanitize($currentCourseID);
