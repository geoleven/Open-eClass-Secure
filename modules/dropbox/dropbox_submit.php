	...
	if (!$error) {
		...
		if (!$error) {
			...
			if (!$error)
			{
				...
				new Dropbox_SentWork($uid, iSanitize($dropbox_title), iSanitize($_POST['description']), iSanitize($_POST['authors']), $dropbox_filename, $dropbox_filesize, $newWorkRecipients);
			}
		}
		chdir ($cwd);
	} //end if(!$error)
...
if (isset($_GET['mailingIndex']))  // examine or send
{
	...
	if (!preg_match($dropbox_cnf["mailingZipRegexp"], $mailing_title, $nameParts))
	{
		...
		//$dropbox_cnf["mailingWhere".$var] = dbSanitize($dropbox_cnf["mailingWhere".$var]);
		...
		if (!$error)
		{
			...
			if ( isset( $_GET['mailingSend']))
			{
				...
				$sendDT = dbSanitize($sendDT);
				$mailing_item->id = dbSanitize($mailing_item->id);
