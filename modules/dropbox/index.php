...
<div id=\"operations_container\">
  <ul id=\"opslist\">
    <li><a href=\"".$_SERVER['SCRIPT_NAME']."?upload=1\">".$dropbox_lang['uploadFile']."</a></li>
  </ul>
</div>";
...
tCont2;
	...
	*/
	else
	{
		...
		$uid = dbSanitize($uid);
		// $dropbox_cnf["userTbl"] = dbSanitize($dropbox_cnf["userTbl"]);
		// $dropbox_cnf[cid] = dbSanitize($dropbox_cnf[cid]);
		// $dropbox_cnf["courseUserTbl"] = dbSanitize($dropbox_cnf["courseUserTbl"]);
 ...
 */
	...
	foreach ($dropbox_person -> receivedWork as $w)
	{
		...
		$tool_content .= "<a href='dropbox_download.php?id=".urlencode($w->id)."' target=_blank>".oSanitize($w->title)."</a>";
