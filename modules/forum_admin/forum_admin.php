...
if(isset($forumgo)) {
	...
	$currentCourseID = dbSanitize($currentCourseID);
	$cat_id = dbSanitize($cat_id);
	...
	if ($result and mysql_num_rows($result) > 0) {
		$forum_id = oSanitize($forum_id);
		$forum_name = oSanitize($forum_name);
		$forum_desc = oSanitize($forum_desc);
		$forum_moderator = oSanitize($forum_moderator);
		$forum_type = oSanitize($forum_type);
		$forum_access = oSanitize($forum_access);
		$cat_id = oSanitize($cat_id);
		...
		while(list($forum_id, $forum_name, $forum_desc, $forum_access,
			$forum_moderator, $forum_type) = mysql_fetch_row($result)) {
				...
				$tool_content .= "\n<td align='center'>
				<a href='$_SERVER[SCRIPT_NAME]?forumgoedit=yes&forum_id=$forum_id&ctg=$ctg&cat_id=$cat_id'>
				<img src='../../template/classic/img/edit.gif' title='$langModify' border='0'></img></a>&nbsp;
				<a href='$_SERVER[SCRIPT_NAME]?forumgodel=yes&forum_id=$forum_id&cat_id=$cat_id&ctg=$ctg&ok=0' onClick='return confirmation();'>
		...
		// $cat_id = oSanitize($cat_id);
		$tool_content .= "
		<form action=\"$_SERVER[SCRIPT_NAME]?forumgoadd=yes&ctg=$ctg&cat_id=$cat_id\" method=\"post\" onsubmit=\"return checkrequired(this,'forum_name');\">
	...
	elseif(isset($forumcatedit)) {
		...
		$cat_title = oSanitize($cat_title);
		$tool_content .= "
  		<form action='$_SERVER[SCRIPT_NAME]?forumcatsave=yes' method=\"post\" onsubmit=\"return checkrequired(this,'cat_title');\">
    		<input type=hidden name=cat_id value=$cat_id>
    		<table width=99% class=\"FormData\">
	...
	elseif (isset($forumcatsave)) {
		$currentCourseID = dbSanitize($currentCourseID);
		$cat_title = dbSanitize($cat_title);
		$cat_id = dbSanitize($cat_id);
		db_query("update catagories set cat_title='$cat_title' where cat_id='$cat_id'", $currentCourseID);
		$cat_id = oSanitize($cat_id);
		$cat_title = oSanitize($cat_title);
		$tool_content .= "\n<p class=\"success_small\">$langNameCatMod<br /><a href=\"$_SERVER[SCRIPT_NAME]?forumadmin=yes\">$langBack</a></p>";
	}
	...
	elseif(isset($forumgosave)) {
		...
		$navigation[]= array ("url"=>"../forum_admin/forum_admin.php", "name"=> $langOrganisation);
		$currentCourseID = dbSanitize($currentCourseID);
		$forum_moderator = dbSanitize($forum_moderator);
		...
		$currentCourseID = dbSanitize($currentCourseID);
		$forum_moderator = dbSanitize($forum_moderator);
		$forum_name = dbSanitize($forum_name);
		$forum_desc = dbSanitize($forum_desc);
		$forum_type = dbSanitize($forum_type);
		$forum_id = dbSanitize($forum_id);
		$cat_id = dbSanitize($cat_id);
	...
	elseif(isset($forumcatadd)) {
		$currentCourseID = dbSanitize($currentCourseID);
		$catagories = dbSanitize($catagories);
	...
	elseif(isset($forumgoadd)) {
		...
		$mysqlMainDb = dbSanitize($mysqlMainDb);
	...
	elseif(isset($forumgodel)) {
		...
		if(isset($forumcatnotify)) { // modify forum category notification
			...
			if ($rows > 0) {
				...
				$forumcatnotify = dbSanitize($forumcatnotify);
				$uid = dbSanitize($uid);
				$cat_id = dbSanitize($cat_id);
				$cours_id = dbSanitize($cours_id);
		...
		$tool_content .= "<form action=\"$_SERVER[SCRIPT_NAME]?forumadmin=yes\" method=\"post\"></td><tr><td>";
		...
		while(list($cat_id, $cat_title) = mysql_fetch_row($result)) {
			...
			<a href='$_SERVER[SCRIPT_NAME]?forumcatedit=yes&cat_id=$cat_id'>
			<img src='../../template/classic/img/edit.gif' border='0' title='$langModify'></img></a>&nbsp;
			<a href='$_SERVER[SCRIPT_NAME]?forumcatdel=yes&cat_id=$cat_id&ok=0' onClick='return confirmation();'>
			<img src='../../template/classic/img/delete.gif' border='0' title='$langDelete'></img></a>
			<a href='$_SERVER[SCRIPT_NAME]?forumcatnotify=$link_notify&cat_id=$cat_id'>	
			<img src='../../template/classic/img/announcements$icon.gif' border='0' title='$langNotify'></img></a>
			</td></tr>";
			$i++;
		}
		$tool_content .= "</tbody></table></form><br/>
		<form action=\"$_SERVER[SCRIPT_NAME]?forumcatadd=yes\" method=\"post\" onsubmit=\"return checkrequired(this,'catagories');\">
		<table width=99% class=\"FormData\" align=\"left\">
		<tbody><tr>
		<th width='220'>&nbsp;</th>
