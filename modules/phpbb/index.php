...
if(isset($forumcatnotify)) { // modify forum category notification
	...
	if($temp == $forum_id){
		...
		if ($rows > 0) {
			...
			$forumnotify = dbSanitize($forumnotify);
			$uid = dbSanitize($uid);
			$forum_id = dbSanitize($forum_id);
			$cours_id = dbSanitize($cours_id);
			$mysqlMainDb = dbSanitize($mysqlMainDb);
...
if ($total_categories) {
	...
	for($i=0; $i < $total_categories; $i++) {
		...
		$catNum = oSanitize($catNum);

		$uid = dbSanitize($uid);
		$catNum = dbSanitize($catNum);
		$cours_id = dbSanitize($cours_id);
		$mysqlMainDb = dbSanitize($mysqlMainDb);
		...
		for ($x=0; $x < count($forum_row); $x++) {
			...
			if ($forum_row[$x]["cat_id"] == $categories[$i]["cat_id"]) {
				...
				$tool_content .= "<td class='Forum_leftside' style='text-align:center'>
					<a href='$_SERVER[SCRIPT_NAME]?forumnotify=$forum_link_notify&amp;forum_id=".iSanitize($forum)."'>	
