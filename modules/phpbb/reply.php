...
if (isset($post_id) && $post_id) {
	...
	$forum = dbSanitize($forum);
	$topic = idbSanitize($topic);
	$post_id = dbSanitize($post_id);
...
$forum_name = iSanitize($myrow["forum_name"]);
$forum_name = oSanitize($forum_name);
$forum_access = iSanitize($myrow["forum_access"]);
$forum_access = oSanitize($forum_access);
$forum_type = iSanitize($myrow["forum_type"]);
$forum_type = oSanitize($forum_type);
$topic_title = iSanitize($myrow["topic_title"]);
$topic_title = oSanitize($topic_title);
$forum_id = iSanitize($forum);
$temp = iSanitize($forum);
if($temp == $forum){
$nameTools = $langReply;
...
if (isset($submit) && $submit) {
	...
	$forum = dbSanitize($forum);
	$uid = dbSanitize($uid);
	$time = dbSanitize($time);
	$poster_ip = dbSanitize($poster_ip);
	$nom = dbSanitize($nom);
	$prenom = dbSanitize($prenom);
	...
	if ($this_post) {
		...
		$this_post = dbSanitize($this_post);
		$message = idbSanitize($message);
	...
	$topic = dbSanitize($topic);
	$forum = dbSanitize($forum);
	$category_id = dbSanitize($category_id);
