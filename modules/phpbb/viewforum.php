...
if(isset($topicnotify)) { // modify topic notification
	...
	$topicnotify = dbSanitize($topicnotify);
...
$forum = dbSanitize($forum);
$first_topic = dbSanitize($first_topic);
$topics_per_page = dbSanitize($topics_per_page);
$currentCourseID = dbSanitize($currentCourseID);
...
if (mysql_num_rows($result) > 0) { // topics found
	while($myrow = mysql_fetch_array($result)) {
		...
		$mysqlMainDb = dbSanitize($mysqlMainDb);
		$cours_id = dbSanitize($cours_id);
		$myrow[topic_id] = dbSanitize($myrow[topic_id]);
		$uid = idbSanitize($uid);
