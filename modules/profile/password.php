...
if (isset($submit) && isset($changePass) && ($changePass == "do")) {
	...
	$sql = "SELECT `nom`,`prenom` ,`username`,`email`,`am` FROM `user`WHERE `user_id`=".dbSanitize($_SESSION["uid"])." ";
	...
	if ($securimage->check($_POST['captcha_code']) == false) {
		header("location:". $_SERVER['SCRIPT_NAME']."?msg=7");
		exit();
	}

	//all checks ok. Change password!
	$mysqlMainDb = dbSanitize($mysqlMainDb);
	$sql = "SELECT `password` FROM `user` WHERE `user_id`=".dbSanitize($_SESSION["uid"])." ";
	$result = db_query($sql, $mysqlMainDb);
	$myrow = mysql_fetch_array($result);
	...
	if($old_pass == $old_pass_db) {
		...
		$mysqlMainDb = dbSanitize($mysqlMainDb);
		$sql = "UPDATE `user` SET `password` = '$new_pass' WHERE `user_id` = ".dbSanitize($_SESSION["uid"])."";
...
if(isset($msg)) {
	...
	switch ($msg){
		...
		case 7: {//not acceptable characters in password
