...
if (isset($submit) && (!isset($ldap_submit)) && !isset($changePass)) {
    ...
    $username_form = idbSanitize($username_form);
	...
	// everything is ok
		...
		$nom_form = idbSanitize($nom_form);
		$prenom_form = idbSanitize($prenom_form);
		$username_form = idbSanitize($username_form);
		$email_form = idbSanitize($email_form);
		$am_form = idbSanitize($am_form);
		...
		if(mysql_query("UPDATE user
	        ...
	        WHERE user_id='".dbSanitize($_SESSION["uid"])."'")) {
...
if (isset($submit) && isset($ldap_submit) && ($ldap_submit == "ON")) {
	...
	mysql_query("UPDATE user SET perso = '$persoStatus',
		lang = '$langcode' WHERE user_id='".dbSanitize($_SESSION["uid"])."' ");
