...
if(@!empty($search_terms)) $or_search_terms = dbSanitize(mysql_real_escape_string($search_terms));
if(@empty($or_search_terms)) $or_search_terms = "";
else $or_search_terms = dbSanitize(mysql_real_escape_string($or_search_terms));
...
{
	$tmp = dbSanitize(explode(" ", $not_search_terms));
	$query .= "-".implode(" -", $tmp);
}
