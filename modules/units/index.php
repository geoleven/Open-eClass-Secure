...
<script type='text/javascript' src='$urlAppend/include/xinha/my_config.js'></script>";
	...
	if ($id = check_admin_unit_resource($res_id)) {
        ...
        $rescomments = dbSanitize($rescomments);
        $restitle = dbSanitize($restitle);
...
$visibility_check = dbSanitize($visibility_check);
...
foreach (array('previous', 'next') as $i) {
        ...
        if ($q and mysql_num_rows($q) > 0) {
                ...
                $link[$i] = "<a href='$_SERVER[SCRIPT_NAME]?id=$q_id'>$arrow1$q_title$arrow2</a>";
        } else {
                $link[$i] = '&nbsp;';
        }
}
...
function check_admin_unit_resource($resource_id)
{
	...
	if ($is_adminOfCourse) {
        $cours_id = dbSanitize($cours_id);
        $resource_id = dbSanitize($resource_id);
...
function actions($res_type, $resource_id, $status)
{
	...
	if (!$is_adminOfCourse) {
        ...
        if ($status != 'del') {
                $content = "<td width='3%'><a href='$_SERVER[SCRIPT_NAME]?edit=$resource_id'>" .
                "<img src='../../template/classic/img/edit.gif' title='$langEdit' /></a></td>";
        } else {
                $content = '<td width="3%">&nbsp;</td>';
        }
        $content .= "<td width='3%'><a href='$_SERVER[SCRIPT_NAME]?del=$resource_id'" .
	...
	if ($status != 'del') {
		if ($res_type == 'text' or $res_type == 'video' or $res_type == 'forum' or $res_type == 'topic') { 
			$content .= "<td width='3%'><a href='$_SERVER[SCRIPT_NAME]?vis=$resource_id'>" .
        ...
        if ($resource_id != $GLOBALS['max_resource_id']) {
                $content .= "<td width='3%'><a href='$_SERVER[SCRIPT_NAME]?down=$resource_id'>" .
                        "<img src='../../template/classic/img/down.gif' title='$langDown'></img></a></td>";
	} else {
		$content .= "<td width='3%'>&nbsp;</td>";
	}
        if (!$first) {
                $content .= "<td width='3%'><a href='$_SERVER[SCRIPT_NAME]?up=$resource_id'>" .
