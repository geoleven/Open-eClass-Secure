...
function insert_docs($id)
{
	...
	foreach ($_POST['document'] as $file_id) {
		...
		$file_id = dbSanitize($file_id);
		$file = mysql_fetch_array(db_query("SELECT * FROM document
			WHERE id =" . intval($file_id), dbSanitize($GLOBALS['currentCourseID'])), MYSQL_ASSOC);
		...
		db_query("INSERT INTO unit_resources SET unit_id=$id, type='doc', title=" .
			 autoquote($title) . ", comments=" . autoquote(dbSanitize($file['comment'])) .
			 ", visibility='dbSanitize($file[visibility])', `order`=$order, `date`=NOW(), res_id=dbSanitize($file[id])",
			 dbSanitize($GLOBALS['mysqlMainDb'])); 
	}
...
function insert_lp($id)
{
	...
	foreach ($_POST['lp'] as $lp_id) {
		...
		if ($lp['visibility'] == 'HIDE') {
			...
			$id = dbSanitize($id);
			$lp['name'] = dbSanitize($lp['name']);
			$order = dbSanitize($order);
			$lp[learnPath_id] = dbSanitize($lp[learnPath_id]);
			$visibility = dbSanitize($visibility);
			$lp['comment'] = dbSanitize($lp['comment']);
...
function insert_video($id)
{
	...
	foreach ($_POST['video'] as $video_id) {
        ...
        $res_id = dbSanitize($res_id);
        $id = dbSanitize($id);
        $table = dbSanitize($table);
...
function insert_work($id)
{
	...
	foreach ($_POST['work'] as $work_id) {
		...
		$id = dbSanitize($id);
		$work['title'] = dbSanitize($work['title']);
		$work['description'] = dbSanitize($work['description']);
		$visibility = dbSanitize($visibility);
		$order = dbSanitize($order);
		$work[id] = dbSanitize($work[id]);
...
function insert_exercise($id)
{
	...
	foreach ($_POST['exercise'] as $exercise_id) {
		...
		$exercise_id = dbSanitize($exercise_id);
		...
		$id = dbSanitize($id);
		$exercise['titre'] = dbSanitize($exercise['titre']);
		$exercise['description'] = dbSanitize($exercise['description']);
		$visibility = dbSanitize($visibility);
		$order = dbSanitize($order);
		$exercise[id] = dbSanitize($exercise[id]);
...
function insert_forum($id)
{
	...
	foreach ($_POST['forum'] as $for_id) {
		...
		if (count($ids) == 2) {
            ...
            $topic_id = dbSanitize($topic_id);
            $forum_id = dbSanitize($forum_id);
            $topic['topic_title'] = dbSanitize($topic['topic_title']);
			$order = dbSanitize($order);
			$topic[topic_id] = dbSanitize($topic[topic_id]);
            ...
            $forum_id = dbSanitize($forum_id);
            $id = dbSanitize($id);
            $forum['forum_name'] = dbSanitize($forum['forum_name']);
            $forum['forum_desc'] = dbSanitize($forum['forum_desc']);
            $order = dbSanitize($order);
            $forum[forum_id] = dbSanitize($forum[forum_id]);
...
function insert_wiki($id)
{
	...
	foreach ($_POST['wiki'] as $wiki_id) {
		...
		$wiki_id = dbSanitize($wiki_id);
		$wiki['title'] = dbSanitize($wiki['title']);
		$id = dbSanitize($id);
		$wiki['description'] = dbSanitize($wiki['description']);
		$order = dbSanitize($order);
