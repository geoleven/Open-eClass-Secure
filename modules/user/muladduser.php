...
$search=array();
if(!empty($search_nom)) {
	$search[] = "u.nom LIKE '".dbSanitize($search_nom)."%'";
}
if(!empty($search_prenom)) {
	$search[] = "u.prenom LIKE '".dbSanitize($search_prenom)."%'";
}
if(!empty($search_uname)) {
	$search[] = "u.username LIKE '".dbSanitize($search_uname)."%'";
}
// added by jexi
if (!empty($users_file)) {
...
function adduser($user, $cid) {
	$result = db_query("SELECT user_id FROM user WHERE username='".dbSanitize($user)."'");
	if (!mysql_num_rows($result))
	return -1;

	list($userid) = mysql_fetch_array($result);
	$userid = dbSanitize($userid);
	$cid = dbSanitize($cid);
