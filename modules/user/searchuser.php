...
if($is_adminOfCourse) {
	...
	if(isset($giveAdmin) && $giveAdmin && $is_adminOfCourse) {
		...
		WHERE user_id='".dbSanitize($_GET['user_id'])."' AND cours_id = $cours_id", $mysqlMainDb);
	}
	// give tutor status
	elseif(isset($giveTutor) && $giveTutor) {
		...
		WHERE user='".dbSanitize($_GET['user_id'])."'", $currentCourseID);
	}
        // remove admin status
        elseif(isset($removeAdmin) && $removeAdmin) {
                ...
                $result = db_query("UPDATE cours_user SET statut = 5
                        WHERE user_id != $uid AND user_id='".dbSanitize($_GET['user_id'])."'
        ...
        elseif(isset($removeTutor) && $removeTutor) {
                ...
                $result = db_query("UPDATE cours_user SET tutor = 0
                        WHERE user_id = '".dbSanitize($_GET['user_id'])."'
                              AND cours_id = $cours_id", $mysqlMainDb);
        }
        ...
        elseif(isset($unregister) && $unregister) {
                ...
                $result = db_query("DELETE FROM cours_user WHERE user_id!= $uid
                        AND user_id = '".dbSanitize($_GET['user_id'])."'
                        AND cours_id = $cours_id", $mysqlMainDb);
	...
	$search=array();
	if(!empty($search_nom)) {
		$search[] = "user.nom LIKE '".idbSanitize($search_nom)."%'";
		$s = "search_nom=$search_nom";
	}
	if(!empty($search_prenom)) {
		$search[] = "user.prenom LIKE '".idbSanitize($search_prenom)."%'";
		$s = "search_prenom=$search_prenom";
	}
	if(!empty($search_uname)) {
		$search[] = "user.username LIKE '".idbSanitize($search_uname)."%'";
		$s = "search_uname=$search_uname";
	}

	$query = join(' AND ', $search);
	$securimage = new Securimage();
	if (!empty($query)&& $securimage->check($_POST['captcha_code']) == true) {
		$cours_id = dbSanitize($cours_id);
