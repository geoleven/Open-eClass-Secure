...
include_once $_SERVER['DOCUMENT_ROOT'] . '/include/cSanitize.php';
...
if ($is_adminOfCourse) {
        ...
        if (isset($_GET['giveAdmin']) && $securimage->check($_POST['captcha_code']) != false ) {
                $new_admin_gid = intval($_GET['giveAdmin']);
                $new_admin_gid = dbSanitize($new_admin_gid);
                $cours_id = dbSanitize($cours_id);
                db_query("UPDATE cours_user SET statut = 1
                                WHERE user_id = $new_admin_gid AND cours_id = $cours_id", $mysqlMainDb);
        } elseif (isset($_GET['giveTutor']) && $securimage->check($_POST['captcha_code']) != false) {
                $new_tutor_gid = intval($_GET['giveTutor']);
                $new_tutor_gid = dbSanitize($new_tutor_gid);
                $cours_id = dbSanitize($cours_id);
        ...
        } elseif (isset($_GET['removeAdmin']) && $securimage->check($_POST['captcha_code']) != false) {
                $removed_admin_gid = intval($_GET['removeAdmin']);
                $removed_admin_gid = dbSanitize($removed_admin_gid);
                $cours_id = dbSanitize($cours_id);
                db_query("UPDATE cours_user SET statut = 5
                                WHERE user_id <> $uid AND
                                      user_id = $removed_admin_gid AND
                                      cours_id = $cours_id", $mysqlMainDb);
        } elseif (isset($_GET['removeTutor']) && $securimage->check($_POST['captcha_code']) != false) {
                $removed_tutor_gid = intval($_GET['removeTutor']);
                $removed_tutor_gid = dbSanitize($removed_tutor_gid);
                $cours_id = dbSanitize($cours_id);
                db_query("UPDATE cours_user SET tutor = 0
                                WHERE user_id = $removed_tutor_gid AND
                                      cours_id = $cours_id", $mysqlMainDb);
        } elseif (isset($_GET['unregister']) && $securimage->check($_POST['captcha_code']) != false) {
                ...
                if ($unregister_ok) {
                    $unregister_gid = dbSanitize($unregister_gid);
                    $cours_id = dbSanitize($cours_id);
...
if ($countUser >= $endList) {
   ...
   <tr>
     <td valign='bottom' align='left' width='20%'>
       <form method='post' action='$_SERVER[SCRIPT_NAME]?numbList=begin'>
	...
	if ($startList!=0) {
		$tool_content .= "
       <form method='post' action='$_SERVER[SCRIPT_NAME]?startList=$startList&amp;numbList=less'>
	...
	$tool_content .= "
     ...
     <td valign='bottom' align='center' width='20%'>
       <form method='post' action='$_SERVER[SCRIPT_NAME]?startList=$startList&amp;numbList=all'>
	...
	if (!((($countUser-$startList)<=$endList) OR ($endList==2000))) {
		$tool_content .= "<form method='post' action='$_SERVER[SCRIPT_NAME]?startList=$startList&amp;numbList=more'>
	...
	$tool_content .= "
     ...
     <td valign='bottom' width='20%'>
       <div align='right'>
       <form method='post' action='$_SERVER[SCRIPT_NAME]?numbList=final'>
...
while ($myrow = mysql_fetch_array($result)) {
        ...
        if(isset($status) && ($status["$currentCourseID"]=='1' OR $status["$currentCourseID"]=='2')) {
                // tutor right
                if ($myrow['tutor'] == '0') {
                        $tool_content .= "<td valign='top' align='center' class='add_user'><a href='$_SERVER[SCRIPT_NAME]?giveTutor=$myrow[user_id]' title='$langGiveTutor'>$langAdd</a></td>";
                } else {
                        $tool_content .= "<td class='highlight' align='center'>$langTutor<br /><a href='$_SERVER[SCRIPT_NAME]?removeTutor=$myrow[user_id]' title='$langRemoveRight'>$langRemove</a></td>";
                }

                // admin right
                if ($myrow['user_id'] != $_SESSION["uid"]) {
                        if ($myrow['statut']=='1') {
                                $tool_content .= "<td class='highlight' align='center'>$langAdministrator<br /><a href='$_SERVER[SCRIPT_NAME]?removeAdmin=$myrow[user_id]' title='$langRemoveRight'>$langRemove</a></td>";
                        } else {
                                $tool_content .= "<td valign='top' align='center' class='add_user'><a href='$_SERVER[SCRIPT_NAME]?giveAdmin=$myrow[user_id]' title='$langGiveAdmin'>$langAdd</a></td>";
                        }
                } else {
                        if ($myrow['statut']=='1') {
                                $tool_content .= "<td valign='top' class='highlight' align='center' title='$langAdmR'><b>$langAdministrator</b></td>";
                        } else {
                                $tool_content .= "<td valign='top' align='center'><a href='$_SERVER[SCRIPT_NAME]?giveAdmin=$myrow[user_id]' >$langGiveAdmin</a></td>";
                        }
                }
                $tool_content .= "<td valign='top' align='center'>";
                $alert_uname = $myrow['prenom'] . " " . $myrow['nom'];
                $tool_content .= "<a href='$_SERVER[SCRIPT_NAME]?unregister=$myrow[user_id]' onClick=\"return confirmation('".addslashes($alert_uname)."');\"><img src='../../template/classic/img/delete.gif' title='$langDelete' /></a>";
        }	// admin only
        $tool_content .= "</td></tr>";$i++;
} 	// end of while
$tool_content .= "<tr>
    <td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td valign='top' align='center'>
    <a href=\"#\" onclick=\"document.getElementById('captcha').src = '../../include/securimage/securimage_show.php?' + Math.random(); return false\">[ Different Image ]</a>
    <br/><img id='captcha' src='../../include/securimage/securimage_show.php' alt='CAPTCHA Image' />
    <form method=\"post\">
